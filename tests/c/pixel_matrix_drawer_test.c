...
void test_draw_char_X(void) {
    ...
    int drawn_width = pixel_matrix_drawer_draw_char(
        ctx, (float) point_zero.x, (float) point_zero.y, character, GColorBlack,
        (float) dot_width, (float) dot_height, (float) gap_h, (float) gap_v,
        align_right, digit_size, DOTTED_TEXT_RENDERING_MODE_PIXEL_PERFECT
    );
